...
- (void)loadData {
    ...
    NSString *headerTitle = [NSString stringWithFormat:@"Digs (%d/%lu)",self.detailIndex+1,(unsigned long)self.detailsDataSource.count];
...
-(NSArray*)loadTagsIntoArray:(NSString*)tags {
    ...
    // This can create individual buttons for each element in the tagsArray
    return tagsArray;
}
...
-(void)retrieveEditedMonologue {
    ...
    if ( [[NSUserDefaults standardUserDefaults] objectForKey:key] ) {
        ...
        self.editArray = [NSArray arrayWithObjects:@"Edit", @"Restore", nil];
    } else {
        self.editArray = [NSArray arrayWithObjects:@"Edit", nil];
    }
    [self.tableView reloadData];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( indexPath.section == monologueTags ) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SEARCHTAGFROMFAVORTIESNOTIFIATION"
                                                            object:object];
        [self segueToBrowseTab];
    }
