...
- (void)loadData {
    ...
    NSString *headerTitle = [NSString stringWithFormat:@"Boneyard (%d/%lu)",self.detailIndex+1,(unsigned long)self.detailsDataSource.count];
...
-(NSArray*)loadTagsIntoArray:(NSString*)tags {
    ...
    tagsArray = [tags componentsSeparatedByCharactersInSet:set];
    return tagsArray;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( indexPath.section == monologueTags) {
        ...
        monologueCell.authorLabel.text = [NSString stringWithFormat:@"%@ %@",currentMonologue.authorFirst, currentMonologue.authorLast];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ( indexPath.section ) {
        case monologueText:
            ...
            NSLog(@"self.prototypeCellText.cellTextLabel is %@",self.prototypeCellText.cellTextLabel);
        ...
        case monologueNotes:
            ...
            NSLog(@"self.prototypeCellText.cellNotesLabel is %@",self.prototypeCellText.cellNotesLabel);
            NSLog(@"monologueNotes size.height is %f",size.height);
            break;
        case monologueTags:
            ...
            NSLog(@"monologueTags size.height is %f",size.height);
            break;
        default:
            size.height = [self.tableView rowHeight];
            NSLog(@"default size.height is %f",size.height);
            break;
    }
...
- (IBAction)favoriteButtonAction:(id)sender {
    NSMutableArray *favoriteMonologues = [[NSMutableArray alloc] init];
    
    // favoriteMonologues = [self loadCustomObjectWithKey:@"favoriteMonologues"];
    favoriteMonologues = [[[NSUserDefaults standardUserDefaults] objectForKey:@"favoriteMonologues"] mutableCopy];
